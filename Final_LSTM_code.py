...
model.eval()
with torch.no_grad():
    predicted_force_profiles = model(whole_x)

y_test_pred = predicted_force_profiles.cpu().data.numpy()
y_max = np.concatenate((y_train_max ,y_test_max))
y_test_pred = denormalize(y_test_pred,y_max)
print(abs(y_test_pred-y).mean())
